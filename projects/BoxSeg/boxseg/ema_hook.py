...
class BoxSegEMAHook(HookBase):
    ...
    def before_train(self):
        ...
        if cur_iter == 0:
            model_without_ddp = self.trainer.model.module if isinstance(self.trainer.model, (nn.parallel.distributed.DistributedDataParallel, nn.DataParallel)) else self.trainer.model
            self.init_param(model_without_ddp)
    ...
    def before_step(self):
        ...
        if cur_iter % self.interval != 0:
            return
        model = self.trainer.model.module if isinstance(self.trainer.model, (nn.parallel.distributed.DistributedDataParallel, nn.DataParallel)) else self.trainer.model
        momentum = min(
