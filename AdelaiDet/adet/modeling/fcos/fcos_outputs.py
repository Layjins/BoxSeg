...
def vfl_loss(pred, target, alpha=0.75, gamma=2.0, iou_weighted=True, reduction='mean'):
...
class FCOSOutputs(nn.Module):
    ...
    def losses(self, logits_pred, reg_pred, ctrness_pred, locations, gt_instances, top_feats=None, use_vfl=False):
        """
        ...
        return self.fcos_losses(instances, use_vfl)

    def fcos_losses(self, instances, use_vfl=False):
        losses, extras = {}, {}
        ...
        logits_pred = instances.logits_pred
        # prepare one_hot
        ...
        if use_vfl:
            pos_ious = ious.clone().detach()
            cls_iou_targets = torch.zeros_like(logits_pred)
        ...
        if use_vfl:
            losses["loss_fcos_vfcls"] = class_loss * self.loss_weight_cls
        else:
    ...
    def predict_proposals(
            ...
            locations, image_sizes, top_feats=None, teacher_with_nms=True
    ):
        if self.training:
