...
class FCOS(nn.Module):
    ...
    def forward(self, images, features, gt_instances=None, top_module=None, use_vfl=False):
        """
        Arguments:
        ...
        if self.training:
            results, losses = self.fcos_outputs.losses(
                ...
                locations, gt_instances, top_feats, use_vfl
            )
    ...
    def forward_test(self, images, features, gt_instances=None, top_module=None, post_nms_topk=100, teacher_with_nms=True):
        features = [features[f] for f in self.in_features]
        locations = self.compute_locations(features)
        logits_pred, reg_pred, ctrness_pred, top_feats, bbox_towers = self.fcos_head(
            features, top_module, self.yield_proposal or self.yield_box_feats
        ...
        results = self.fcos_outputs.predict_proposals(
            logits_pred, reg_pred, ctrness_pred,
            locations, images.image_sizes, top_feats, teacher_with_nms
        )
